...
객체 지향 프로그래밍(OOP)을 활용하여 클래스로 코드를 구조화하였으며, JSON 파일 입출력을 통해 프로그램 종료 후에도 데이터(퀴즈 목록, 최고 점수, 플레이 기록)를 영구적으로 보존합니다.
...
- **외부 라이브러 금지**: `json`, `os`, `sys`, `random`, `datetime` 등 파이썬 표준 라이브러리만을 활용하여 구현했습니다.
- **클래스 역할 분리 (최소 2개 이상)**: 
  - `Quiz` 클래스: 개별 퀴즈 데이터를 담고 관리 (JSON 직렬화 담당)
  - `QuizGame` 클래스: 게임의 상태(퀴즈 리스트, 점수, 히스토리) 관리 및 전반적인 비즈니스 로직(출제, 저장, 추가, 삭제) 담당
...
## 5. 핵심 기능 목록 (기본 + 보너스 완벽 구현)
- **퀴즈 풀기**: 저장된 퀴즈를 순서대로 풀고 정답을 채점합니다.
- **퀴즈 추가**: 새로운 퀴즈 내용(문제, 4지선다, 정답, 힌트)을 터미널에서 직접 등록할 수 있습니다.
- **퀴즈 목록 조회**: 현재 등록되어 있는 퀴즈들의 목록을 한눈에 확인합니다.
- **퀴즈 삭제 (보너스)**: 원치 않는 퀴즈를 번호로 선택해 삭제할 수 있습니다.
- **점수 및 기록 확인 (보너스)**: 역대 최고 점수와 함께 최근 5번의 플레이 기록(날짜, 푼 문제 수, 점수)을 확인합니다.
- **문제 수 선택 및 랜덤 출제 (보너스)**: 퀴즈 시작 전 원하는 문제 수를 입력받아 매번 무작위 순서로 출제합니다.
- **힌트 및 점수 차감 시스템 (보너스)**: 문제 풀이 중 `0`번을 입력하면 1점을 차감하고 문제에 대한 힌트를 볼 수 있습니다. (맞춘 문제가 0개일 때는 힌트 보기 제한)
...
codyssey-02-Quiz
 ...
 ┣ state.json     # 퀴즈 데이터, 최고 점수, 플레이 기록이 저장되는 파일
...
| 1. 퀴즈 풀기 및 힌트 사용 | 2. 퀴즈 추가 |
| :---: | :---: |
| ![퀴즈 풀기](assets/05_QUIZ_1.png) | ![퀴즈 추가](assets/06_QUIZ_2.png) |
| **3. 퀴즈 삭제 및 목록 확인** | **4. 점수 및 기록 히스토리 확인** |
